...
HingeJoint::HingeJoint(int bodyA,
                       int bodyB,
...
void HingeJoint::updateState(int idxA,
                             int idxB,
...
void HingeJoint::computeC(const Eigen::VectorXd& x, Eigen::VectorXd& C) const {
    ...
    const JointPrimitives::AxisCrossPrimitive axis_cross{axisA_, axisB_};

    C.segment<3>(0) = point.value(evalA, evalB);
    C.segment<3>(3) = axis_cross.value(evalA, evalB);
}
...
void HingeJoint::computeJ(const Eigen::VectorXd& x, Eigen::SparseMatrix<double>& J) const {
    ...
    axis_cross.addJacobianRows(3, stateA, stateB, evalA, evalB, triplets);
